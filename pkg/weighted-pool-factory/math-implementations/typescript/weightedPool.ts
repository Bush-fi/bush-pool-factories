// Exact fixed-point port of Bush's WeightedPool.sol (pool-weighted/contracts/WeightedPool.sol) on top of the
// WeightedMath port in ./weightedMath.ts. Implements the bush-maths `PoolBase` interface so the bush-maths Vault
// flow (scaling, rates, fees, hooks) can run it: see pvt/math-check/README.md.
//
// Differences from a plain WeightedMath wrapper that matter for bit-exactness with the deployed contract:
//   - onSwap rounds the token-in balance up by 1 wei before calling the maths;
//   - every operation enforces the per-token minimum balance (MinTokenBalanceLib), which reverts with
//     TokenBalanceBelowMin on-chain.

import { MAX_UINT256 } from '@bush.fi/maths/constants';
import { MathSol } from '@bush.fi/maths/utils/math';
import { toRawUndoRateRoundDown } from '@bush.fi/maths/vault/utils';
import {
    ...
    SwapKind,
    type SwapParams,
} from '@bush.fi/maths/vault/types';
import {
