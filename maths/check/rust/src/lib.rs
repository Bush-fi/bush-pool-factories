//! Support for the `math_check` binary (main.rs): reads a test-data file, runs every swap / add / remove through
//! the bush-maths Vault flow with the `PoolBase` (and optional `HookBase`) built by the registry in main.rs, and
//! prints the outcomes as JSON for `maths:check` to report.

pub use alloy_primitives::U256;
pub use bush_maths;
pub use bush_maths::common::json::PoolJson;
use bush_maths::common::types::{
    AddLiquidityInput, AddLiquidityKind, PoolState, RemoveLiquidityInput, RemoveLiquidityKind,
    SwapInput, SwapKind,
};
...
/// Entry point of the `math_check` binary: `math_check <test-data-file.json>`.
