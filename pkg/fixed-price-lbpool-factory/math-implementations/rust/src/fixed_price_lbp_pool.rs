//! Exact fixed-point port of Bush's FixedPriceLBPool.sol: a constant-sum sale pool where one project token always
//! costs `project_token_rate` reserve tokens, only the project token can be bought, and only during the sale window.
//! See pvt/math-check/README.md.

//! FixedPriceLBP pool implementation

use bush_maths::common::errors::PoolError;
use bush_maths::common::maths::{div_down_fixed, mul_down_fixed, mul_up_fixed};
use bush_maths::common::pool_base::PoolBase;
use bush_maths::common::types::{Rounding, SwapKind, SwapParams};
use bush_maths::pools::fixed_price_lbp::fixed_price_lbp_data::FixedPriceLBPState;
