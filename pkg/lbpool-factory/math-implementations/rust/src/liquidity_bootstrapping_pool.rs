//! Exact fixed-point port of Bush's LBPool.sol: a two-token WeightedPool whose weights move linearly between a
//! start and an end weight during the sale (GradualValueChange), with swaps only enabled during the sale and
//! optionally blocked for selling the project token back in. The weighted maths is reused from bush-maths; the
//! weight schedule is ported in liquidity_bootstrapping_math.rs. Only seeded LBPs (no reserve virtual balance) are
//! modelled. See pvt/math-check/README.md.
...
use bush_maths::pools::liquidity_bootstrapping::liquidity_bootstrapping_data::LiquidityBootstrappingState;
use crate::liquidity_bootstrapping_math::get_normalized_weights;
use bush_maths::pools::weighted::weighted_pool::{compute_min_token_balances, WeightedPool};
use bush_maths::pools::weighted::weighted_math::{MAX_INVARIANT_RATIO, MIN_INVARIANT_RATIO};
