...
import { bn, decimal, fp, fromFp, toFp, fpDivDown, fpMulDown, FP_ONE, FP_100_PCT } from '../numbers';
...
export function computeBPTSwapFeeAmount(
  ...
  const supplyGrowthRatio = fpDivDown(postSupply, preSupply);

  if (bn(fpInvariantGrowthRatio) <= supplyGrowthRatio) {
    return bn(0);
  }
  const swapFeePercentage = FP_100_PCT - fpDivDown(supplyGrowthRatio, fpInvariantGrowthRatio);
  const k = fpMulDown(swapFeePercentage, fpProtocolSwapFeePercentage);
