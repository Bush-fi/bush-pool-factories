// Exact fixed-point port of Bush's StableSurgeHook.sol (pool-hooks/contracts/StableSurgeHook.sol +
// SurgeHookCommon.sol): a dynamic swap fee that rises linearly once a swap pushes the pool's imbalance past a
// threshold, and a guard that blocks unbalanced liquidity operations while surging. The pool itself is a plain
// StablePool, reused from bush-maths. See pvt/math-check/README.md.

import { HookBase, HookStateBase } from '@bush.fi/maths/hooks/types';
import { MathSol } from '@bush.fi/maths/utils/math';
import { AddKind, RemoveKind, SwapKind, SwapParams } from '@bush.fi/maths/vault/types';
