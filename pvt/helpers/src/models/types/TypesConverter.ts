...
import { IVault, IVault__factory, Vault, VaultMock } from '@bush.fi/v3-vault/typechain-types';
import { IERC20, IERC20__factory, IVaultMock, IVaultMock__factory } from '@bush.fi/v3-interfaces/typechain-types';
import { IAuthentication } from '@bush.fi/v3-solidity-utils/typechain-types';
import { IAuthentication__factory } from '@bush.fi/v3-pool-weighted/typechain-types';
...
export default {
  ...
  toAddress(to?: Account): string {
    ...
    // ethers v6 contracts expose their address as `target`.
    if (to instanceof BaseContract) return to.target as string;
    return to.address;
  },
