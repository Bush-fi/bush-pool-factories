...
WeightedMath port in weighted_math.py. Implements the bush-maths `PoolBase` interface so the bush-maths Vault flow
(scaling, rates, fees, hooks) can run it: see pvt/math-check/README.md.
...
`pool_state` needs: weights, scaling_factors and (optionally) min_token_balances.
"""
